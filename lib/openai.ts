...
- ⭐ 난이도: 관광객이 아닌, 미국 생활을 조금 아는 사람도 모를 만한 인사이더 지식
  - ❌ 금지: "미국은 팁 문화가 있어", "미국은 땅이 커", "할로윈에 trick or treat 해" 같은 기초 상식
  - ✅ 좋은 예: "Venmo 결제 시 이모지를 넣는 게 매너", "Target의 Up&Up 브랜드가 가성비 끝판왕인 이유", "미국 약국(CVS/Walgreens)에서 처방전 없이 살 수 있는 약의 범위", "Costco 리턴 정책의 미친 관대함", "미국 아파트 렌트 시 credit score가 왜 중요한지"
...
- 두 용어는 줄바꿈(\\n\\n)으로 구분
- 예시:
"Rain Check: 마트에서 세일 상품이 품절됐을 때 직원한테 'rain check 주세요' 하면 나중에 같은 세일가로 살 수 있는 보증서를 줘. 한국엔 없는 제도라 모르는 사람이 많은데, 미국에선 꽤 흔한 소비자 권리야.\\n\\nPrice Match: Target이나 Walmart에서는 경쟁사 가격이 더 싸면 그 가격에 맞춰주는 'price match' 정책이 있어. 계산대에서 아마존 가격 보여주면 그 자리에서 할인해주는 거지. 이걸 모르면 미국에서 쇼핑할 때 손해 보는 셈이야."
...
- Total 90-120 words across all 3 scenes
...
[✓] 스크립트 **bold** 5개 = expressions 배열 5개 (정확히 일치, 누락/추가 없음)
